...
const ProviderCard = ({ provider, gameCount }) => {
  ...
  const [actualGameCount, setActualGameCount] = useState(gameCount || 0);
  ...
  console.log("isHoveringCard",isHoveringCard);
  ...
  useEffect(() => {
    if (provider?.provider) {
      dispatch(getGames({ provider: provider.provider })).then((action) => {
        if (action?.payload?.data) {
          setActualGameCount(action.payload.data.length);
        }
      });
    }
    ...
    <Link to={`/provider/${provider.provider}`}>
      ...
      >
        {/* ================= BACKGROUND IMAGE (INITIAL STATE) ================= */}
          {provider ? (
           <img
              src={provider?.img || 'https://i.ytimg.com/vi/5S_W1oK5IEs/maxresdefault.jpg'}
              alt=""
